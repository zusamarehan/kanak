...
<head>
    ...
    <title>Record Donation — Kanak Foundation</title>
    <meta name="description" content="Enter and record donations from donors for Kanak Foundation.">
...
<body>
    ...
    <div class="container">
        ...
        <div class="top-bar-mobile" style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 2rem; border-bottom: 1px solid var(--border-glow);">
            ...
            <nav style="display: flex; gap: 2rem;">
                ...
                <a href="/donation-entry" class="active" style="color: var(--text-main); text-decoration: none; font-size: 0.9rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em; position: relative; padding-bottom: 1rem; line-height: 1;">
                    Record Donation
        ...
        <!-- Form Card -->
        <div class="form-card" id="donation-form-card">
            <div class="form-card-header">
                <h1>Record Donation</h1>
                <p>Enter a new donation from a donor below.</p>
            </div>
            ...
            <form id="donation-form" novalidate>
                ...
                <div class="form-group">
                    ...
                    <div class="select-wrapper">
                        <select id="donor_id" class="form-select" required>
                            <option value="" disabled selected>Loading donors…</option>
                        </select>
                    </div>
                    <div class="field-error" id="donor-error">Please select a donor.</div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('donor_id').addEventListener('change', () => clearError('donor'));
        ...
        async function loadDonors() {
            try {
                ...
                if (data.success && data.data.length > 0) {
                    select.innerHTML = '<option value="" disabled selected>Select a donor…</option>';
                    ...
                    select.innerHTML = '<option value="" disabled selected>No donors found</option>';
                }
            } catch (err) {
                console.error('Failed to load donors', err);
                document.getElementById('donor_id').innerHTML = '<option value="" disabled selected>Error loading donors</option>';
            }
        }
        ...
        async function handleSubmit(e) {
            ...
            try {
                ...
                if (res.ok && data.success) {
                    showToast('success', `Donation of ${formatter.format(data.data.amount)} recorded for ${data.data.donor_name}.`);
                    addRecentEntry(data.data);

                    // Reset form
                    document.getElementById('amount').value = '';
                    document.getElementById('donor_id').selectedIndex = 0;
        ...
        function addRecentEntry(entry) {
            ...
            const initials = (entry.donor_name || 'AU').substring(0, 2).toUpperCase();
            const monthNames = ['', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
            const dateLabel = `${monthNames[entry.month]} ${entry.year}`;
            ...
            item.innerHTML = `
                <div class="entry-left">
                    ...
                    <div>
                        <div class="entry-name">${entry.donor_name}</div>
